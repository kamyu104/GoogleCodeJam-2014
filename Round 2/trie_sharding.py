...
def count(cnts, n):  # Time: O(N^2)
    ...
    for i in xrange(1, n+1):
        ...
        for j in xrange(1, i):
            dp[i] = submod(dp[i], mulmod(dp[j], nCr(i, i-j)))  # substract count of j non-empty with (i-j) empty servers for j in [1, i-1]
    return dp[n]
...
def trie_sharding():
    ...
    for i in reversed(xrange(len(trie))):  # O(T) times
        ...
        cnts = [1] if i in end_nodes else []
        for j in trie[i].itervalues():
            cnts.append(cnt[j])
            cnt[i] = min(cnt[i]+cnt[j], N)
        result += cnt[i]
